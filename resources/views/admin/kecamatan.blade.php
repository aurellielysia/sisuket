                ...
                <tbody>
                    @foreach($kecamatan as $index => $kecamatan)
                        <tr class="bg-white border-b hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4 font-semibold text-gray-900 border-r border-gray-200">{{ $kecamatan->nama_kecamatan }}</td>
                            <td class="px-6 py-4">
                                <form action="{{ route('admin.kecamatan.destroy', $kecamatan->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus barang ini?');">
                                    @csrf
                                    @method('DELETE')
                ...
                <div class="relative bg-white rounded-lg shadow">
                    ...
                    <div class="p-4 md:p-5">
                        <div class="grid gap-4 mb-4 grid-cols-2">
                            ...
                            <label for="nama_kecamatan" class="block mb-2 text-sm font-medium text-gray-900">Nama Kecamatan</label>
                    <input type="text" name="nama_kecamatan" id="nama_kecamatan" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5" placeholder="Type kecamatan name" required>
                    </div>
                    </div>
